...
/**
 * 应用程序全局的通用工具类，功能比较单一，经常被复用的功能，
 * 应该封装到此工具类当中，从而给全局代码提供方面的操作。
 *
 */
object GlobalUtil {
    ...
    val appPackage: String
        get() = FunsEyeApplication.context.packageName
     ...
     */
    val appName: String
        get() = FunsEyeApplication.context.resources.getString(FunsEyeApplication.context.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = FunsEyeApplication.context.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            FunsEyeApplication.context.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            FunsEyeApplication.context.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(FunsEyeApplication.context.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
    ...
    fun getString(resId: Int): String {
        return FunsEyeApplication.context.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return FunsEyeApplication.context.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return FunsEyeApplication.context.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = FunsEyeApplication.context.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            FunsEyeApplication.context.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = FunsEyeApplication.context.packageManager
